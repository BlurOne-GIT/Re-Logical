...
public class Spinner : GameTile, IReloadable, IFixable, IOverlayable
{
    ...
    public void Reload(ITile[,] tiles)
    {
        ...
        [
            ...
            Point.Y != 0 && !ITile.VerticalAttachables.Contains(tiles[Point.X, Point.Y - 1].FileValue),   // Up
        ...
        {
            if (Point.Y != 0 && tiles[Point.X, Point.Y - 1].FileValue is not (byte)ITile.TileTypes.Dropper)
            {
        ...
        if (!_closedPipes[(int)Direction.Down] && tiles[Point.X, Point.Y + 1].FileValue is not (byte)ITile.TileTypes.Dropper)
        {
